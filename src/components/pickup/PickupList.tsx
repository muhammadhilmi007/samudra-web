...
const PickupList: React.FC<PickupListProps> = ({
  ...
  const { pickup: pickups, loading } = useSelector(
  ...
  const filteredPickups = useMemo(() => {
    return pickups.filter((pickup: Pickup) => {
      ...
      if (searchTerm) {
        ...
        const matchesSender = getCustomerName(pickup.pengirimId)
          ?.toLowerCase()
          .includes(search);
        const matchesDriver = getEmployeeName(pickup.supirId)
          ?.toLowerCase()
          .includes(search);
                      ...
                      return (
                        <TableRow key={pickup._id}>
                          <TableCell className="font-medium">
                            {pickup.noPengambilan}
                          </TableCell>
                          <TableCell>{formatDate(pickup.tanggal)}</TableCell>
                          <TableCell>
                            {getBranchName(pickup.cabangId)}
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            ...
                            <div className="text-xs text-muted-foreground">
                              {vehicleInfo.namaKendaraan}
                            </div>
                          </TableCell>
                          <TableCell>
                            <div>{getEmployeeName(pickup.supirId)}</div>
                            <div className="text-xs text-muted-foreground">
                              {getEmployeeName(pickup.kenekId)}
                            </div>
                          </TableCell>
                          <TableCell>
                            {pickup.waktuPulang ? (
            ...
            {filteredPickups.length > 0 && (
              <div className="flex justify-between items-center mt-4">
                <div className="text-sm text-muted-foreground">
                  ...
                  {Math.min(startIndex + itemsPerPage, filteredPickups.length)}{" "}
                  dari {filteredPickups.length} data
                </div>
                <Pagination>
                  <PaginationContent>
                    <PaginationItem>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {selectedPickup?.noPengambilan}? Tindakan ini tidak dapat
              dibatalkan.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
